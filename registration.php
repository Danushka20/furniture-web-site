    ...
    <div class="holder holder-form">
        ...
        if (isset($_POST["submit"])) {
           ...
           if (count($errors)>0) {
            ...
            $sql = "INSERT INTO users (full_name, surname, address, phone_number, email, password) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_stmt_init($con);
            if (mysqli_stmt_prepare($stmt, $sql)) {
                mysqli_stmt_bind_param($stmt, "ssssss", $fullName, $surname, $address, $phone_number, $email, $passwordHash);
